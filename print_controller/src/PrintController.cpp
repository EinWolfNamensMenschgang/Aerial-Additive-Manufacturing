...
#define PRINT_VELOCITY 0.002
#define NON_PRINT_VELOCITY 0.002
#define ACCURACY_DISTANCE 0.0005
...
public:
    PathFollowerNode()
        ...
        odomSubscription_ = create_subscription<nav_msgs::msg::Odometry>(
            "/drone1/odom", 10, std::bind(&PathFollowerNode::onOdom, this, std::placeholders::_1));
...
private:
    ...
    void onOdom(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        if (distance < ACCURACY_DISTANCE) {
            ...
            if(isExtruding_){
                normalizedLinearVelX = normalizedLinearVelX * PRINT_VELOCITY;
                normalizedLinearVelY = normalizedLinearVelY * PRINT_VELOCITY;
                normalizedLinearVelZ = normalizedLinearVelZ * PRINT_VELOCITY;
            }else{
                normalizedLinearVelX = normalizedLinearVelX * NON_PRINT_VELOCITY;
                normalizedLinearVelY = normalizedLinearVelY * NON_PRINT_VELOCITY;
                normalizedLinearVelZ = normalizedLinearVelZ * NON_PRINT_VELOCITY;
            }

            geometry_msgs::msg::Twist twistMsg;
            twistMsg.linear.x = normalizedLinearVelX;
            twistMsg.linear.y = normalizedLinearVelY;
            twistMsg.linear.z = normalizedLinearVelZ;
